...
double jboyd_Q2_SBS8 = 4.368; //4.368;
...
double jboyd_GMn_SBS4 = 0.9772; //0.9634; //0.9772; //0.978484; //1.00108; //0.9882; //0.963793; //0.9594; //using NonWeighted //weighted: .9473; //0.986481; //0.88548169; //0.870183447; //0.875815108; //0.856942989; //0.870183447; //0.868644662; //0.87007995;
double jboyd_GMn_SBS4_syst_error = 0.0154; //0.0113; //0.0154; //0.015860000; //using NonWeighted //weighted:0.00422; //0.0653; //0.00915483;//0.001182874; //0.010842118; //0.050072968; //0.000447308; //0.041910973; //0.044067905; //0.05056551;
double jboyd_GMn_SBS4_stat_error = 0.0100; //0.0085; //0.0100; //0.00818; //0.013068300; //using NonWeighted //weighted:0.000717; //0.0653; //0.00915483;//0.001182874; //0.010842118; //0.050072968; //0.000447308; //0.041910973; //0.044067905; //0.05056551;

//-------------------------------
	// SBS8
//-------------------------------

double jboyd_GMn_SBS8 =  0.9323;// 0.9283; //0.9282; //0.9354; //0.900931; //0.9294; //using NonWeighted //weighted:0.8992; //0.8659; //0.887; //0.908872683; //0.869313056; //0.870183447; //0.875815108; //0.856942989; //0.870183447; //0.868644662; //0.87007995;
double jboyd_GMn_SBS8_syst_error = 0.0174; //0.01900000; //0.018900000; //using NonWeighted //weighted:0.0156; //0.00915483; //0.00618213; //0.010842118; //0.050072968; //0.000447308; //0.041910973; //0.044067905; //0.05056551;
double jboyd_GMn_SBS8_stat_error = 0.0066; //0.00755088; //0.012095000; //using NonWeighted //weighted:0.0079; //0.064; //0.00915483; //0.00618213; //0.010842118; //0.050072968; //0.000447308; //0.041910973; //0.044067905; //0.05056551;

	//----------------------------------

double SBS8_yield_ratio = 0.3369; //0.3256; //0.3310; //0.3322;
double SBS8_yield_ratio_syst_error = 0.0170; //0.0325; //0.0121;
double SBS8_yield_ratio_stat_error = 0.0056; //0.00654; //0.0339; //0.0038;

double SBS8_scale_factor_ratio = 0.9716; //0.970; //1.0193; //1.0297778;
double SBS8_scale_factor_ratio_syst_error = 0.0110; //0.0384; // 0.037583;
double SBS8_scale_factor_ratio_stat_error = 0.0058; //0.0070; //0.0589; //0.0065;
...
double jboyd_GMn_SBS9 = 0.918297; //0.921297; //0.913998; //0.921297; //0.925995; //0.921297; //0.950821; //1.0246; //1.00921; //0.9687; // 0.9587; //using NonWeighted //weighted:0.9516; //0.968790674; //0.867767465; //0.88564948; //0.89487207; //0.867767465; //0.88564948; //0.893231519; //0.925263924; //0.91079894;
double jboyd_GMn_SBS9_syst_error = 0.0117; //0.0182285000; //2*0.0081; //using NonWeighted //weighted:0.00686; //0.0084 //0.0091544; //0.00855204; //0.011112426; //0.050095563; //0.000574477; //0.042572282; //0.043045523; //0.043729448; //0.05056551;
double jboyd_GMn_SBS9_stat_error = 0.0032; //0.0032800000; // 0.002969; //using NonWeighted //weighted:0.004762; //0.0929; //0.0091544; //0.00855204; //0.011112426; //0.050095563; //0.000574477; //0.042572282; //0.043045523; //0.043729448; //0.05056551;

	//----------------------------------

double SBS9_yield_ratio = 0.3703; //0.3759; //0.4157; //0.3721;
double SBS9_yield_ratio_syst_error = 0.0059; //0.0091; //0.0061;
double SBS9_yield_ratio_stat_error = 0.0045; //0.0010; //0.0060; //0.0008;
	//----------------------------------
double SBS9_scale_factor_ratio = 1.013; //1.2269; //1.248; //1.2269; //1.1284444;
double SBS9_scale_factor_ratio_syst_error = 0.0175; //0.0267; //0.018281;
double SBS9_scale_factor_ratio_stat_error = 0.0033; //0.0012; //0.0078; //0.0013;
//-------------------------------
//-------------------------------
...
void plot_jboyd_data(bool bool_plot_jboyd_GMn4 = false, bool use_q2_error = false){
...
// We will add the stat error to the syst error so that it hopefully pops out further than the syst. alone
	...
	if( use_q2_error == false ){
		jboyd_Q2_error_vec = {0.0, 0.0, 0.0};
	}
	...
	tge_GMn_SBS8_jboyd->SetMarkerColor(kCyan);
	tge_GMn_SBS8_jboyd->SetLineColor(kCyan);
	...
	tge_GMn_SBS9_jboyd->SetMarkerColor(kMagenta);
	tge_GMn_SBS9_jboyd->SetLineColor(kMagenta);
	...
	tge_GMn_SBS4_jboyd->SetMarkerColor(kTeal-9);
...
void general_plot_jboyd( TCanvas *c_to_plot_on, bool plot_jboyd_GMn4 = true, int sbs4color = 8, int sbs8color = 880, int sbs9color = 600, int customMarkerStyle = 20 ){
	c_to_plot_on->cd();

	plot_jboyd_data(plot_jboyd_GMn4);
	...
	if( plot_jboyd_GMn4 ){
		...
		if( tleg_GMn_jboyd != NULL ){
			TString label_gmn_nominal = Form("SBS4 G_{M}^{n} = %.4f, Q^{2} = %0.3f", jboyd_GMn_vec[2], jboyd_Q2_vec[2]);
			if( doesLegendEntryExist( tleg_GMn_jboyd, label_gmn_nominal) ){
	...
	if( tleg_GMn_jboyd != NULL ){
		TString label_gmn_nominal = Form("SBS8 G_{M}^{n} = %.4f, Q^{2} = %0.3f", jboyd_GMn_vec[0], jboyd_Q2_vec[0]);
		if( doesLegendEntryExist( tleg_GMn_jboyd, label_gmn_nominal) ){
	...
	tge_GMn_SBS8_jboyd_stat->SetMarkerColor(sbs8color);
	tge_GMn_SBS8_jboyd_stat->SetMarkerStyle(customMarkerStyle);

	tge_GMn_SBS9_jboyd_stat->SetLineColor(sbs9color);
	tge_GMn_SBS9_jboyd_stat->SetMarkerStyle(customMarkerStyle);

	tge_GMn_SBS4_jboyd_stat->SetMarkerColor(sbs4color);
	tge_GMn_SBS4_jboyd_stat->SetMarkerStyle(customMarkerStyle);

	// tge_GMn_SBS8_jboyd->SetMarkerColor(sbs8color);
	tge_GMn_SBS8_jboyd->SetMarkerStyle(customMarkerStyle);

	// tge_GMn_SBS9_jboyd->SetLineColor(sbs9color);
	tge_GMn_SBS9_jboyd->SetMarkerStyle(customMarkerStyle);

	// tge_GMn_SBS4_jboyd->SetMarkerColor(sbs4color);
	tge_GMn_SBS4_jboyd->SetMarkerStyle(customMarkerStyle);	
}
