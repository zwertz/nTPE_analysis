...
//constructor to handle user-input (essentially anticut or MC inelastic) backgrounds
fit_histogram::fit_histogram(TH1D *h_data,TH1D *h_p,TH1D *h_n, TH1D *h_bkgd, const char* fit_name, const char* fit_type, int param_count, double xmin, double xmax,const  string& fit_options){
        //First check that the histograms are not null
        if(!h_data || !h_p || !h_n || !h_bkgd){
        cerr << "At least one of the histograms are null in the constructor!" << endl;
        }
        ...
        //check that the fitType is implemented or not. Will need to update that if more fit types/functions are implemented
        if(fitType != "fitFullShift_InterpolateBG" && fitType !="fitFullShiftNoBG" ){
        cerr << "The fitType: " << fitType << " is not properly implemented! Find the error or implement the fit!" << endl;
        }
        ...
        hist_p = (TH1D*) h_p->Clone(Form("h_dx_mc_p_%s",fitName.c_str()));
        hist_n = (TH1D*) h_n->Clone(Form("h_dx_mc_n_%s",fitName.c_str()));
	hist_bkgd = (TH1D*) h_bkgd->Clone(Form("h_dx_bkgd_%s",fitName.c_str()));
        paramCount = param_count;
        xMin = xmin;
        xMax = xmax;
        fitOptions = fit_options;
...
fit_histogram::fit_histogram(TH1D *h_data,TH1D *h_p,TH1D *h_n, const char* fit_name, const char* fit_type, int param_count, double xmin, double xmax,const string& fit_options){
 ...
 //First check that the histograms are not null
        ...
        //check that the fitType is implemented or not. Will need to update that if more fit types/functions are implemented
        if(fitType != "fitFullShift_gaussBG" && fitType !="fitFullShiftNoBG"){
        cerr << "The fitType: " << fitType << " is not properly implemented! Find the error or implement the fit!" << endl;
        }
	...
	//Now fit the data histo with our function with a gauss background
        vector<pair<double,double>> paramsErrsChi = fitAndFineFit(hist_data,fitName,fitType,paramCount,xMin,xMax,fitOptions);
	...
	//Work here
        //Handle the background parameters. This should be compatible with Gaussian
        for(int j = 4; j < paramCount; j++){
        bkgd_params.push_back(paramsErrsChi[j].first);
        bkgd_param_errs.push_back(paramsErrsChi[j].second);
        }
...
//The total function is the proton and neutron peaks + a Nth order polynomial for background
//This is equivalent to neutron_scale*neutron + proton_scale*proton + background
double fitFullshift = proton_scale*( R_sf*neutron + proton) + polyN_fit(x, &param[4]);
return fitFullshift;
}

double fit_histogram::fitFullShift_gaussBG(double *x, double *param){
//MC float parameters
double proton_scale = param[0];
double R_sf = param[1];
...
double gauss_amp = param[4];
double gauss_mean = param[5];
double gauss_sigma = param[6];

double gauss_BG = gauss_amp*TMath::Exp(-0.5 * TMath::Power((x[0]-gauss_mean),2.0)/TMath::Power(gauss_sigma,2.0));

//The total function is the proton and neutron peaks + a Gaussian distribution for background
//This is equivalent to neutron_scale*neutron + proton_scale*proton + background
double fitFullshift = proton_scale*( R_sf*neutron + proton) + gauss_BG;
return fitFullshift;
}
...
double fit_histogram::fitFullShift_InterpolateBG(double *x, double *param){
//MC float parameters
double proton_scale = param[0];
double R_sf = param[1];
...
//For bkgd
double bkgd = hist_bkgd->Interpolate(x[0]);

//This is equivalent to neutron_scale*neutron + proton_scale*proton + background
double val = proton_scale*( R_sf*neutron + proton) + bg_scale * bkgd;
...
double fit_histogram::InterpolateBG(double *x, double *param){

double bg_scale = param[0];
//For bkgd
double bkgd = hist_bkgd->Interpolate(x[0]);

double val = bg_scale * bkgd;
...
double fit_histogram::Gauss(double *x, double *param){

double gauss_amp = param[0];
double gauss_mean = param[1];
double gauss_sigma = param[2];
...
TF1 *datFit = new TF1(fitName.c_str(),[this,fitFormula](double *x, double *par) -> double {
	...
	}else if(fitFormula =="fitFullShift_gaussBG"){
	return this->fitFullShift_gaussBG(x, par);
	}else if(fitFormula =="fitFullShift_InterpolateBG"){
	return this->fitFullShift_InterpolateBG(x, par);
	}else{
...
string poly_string("poly");
string gauss_string("gauss");
//check if the fitFormula has shifted parameters
	...
	if(fitFormula.find(poly_string) != string::npos){
	...
	if(fitFormula.find(gauss_string) != string::npos){
	//datFit->SetParLimits(4,0.0000000001,10000);
	datFit->SetParLimits(6,0.000000001,100000);
	}
